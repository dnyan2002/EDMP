...
class GreetingServer3(Thread): #
    ...
    def handle_client(self, client_socket, addr):
        ...
        print(f"✅ Connection established with 4th : {addr}") #
        ...
        while self.status:
            try:
                ...
                if not raw_data:  # Check if connection was closed
                    print(f"⚠️ Client 4th : {addr} disconnected - connection closed") #
                    break
                    
                print(f"📩 Raw data received from 4th :{addr}: {raw_data!r}") #
                try:
                    client_message = raw_data.decode().strip()
                    print(f"📩 Decoded message from 4th :{addr}: {client_message}") #
                except UnicodeDecodeError as e:
                    print(f"⚠️ Cannot decode data from 4th : {addr}: {e}")
                    print(f"⚠️ Attempting to process as binary data")
                    # Handle binary data if needed
                    continue

                if not client_message:
                    print(f"⚠️ Client 4th : {addr} sent empty message but connection maintained")
                    continue  # Continue instead of breaking on empty message

                print(f"📩 Received from 4th :{addr}: {client_message}") #

                # Process and store data
                # for update connection if IOT is On

                self.database_communication.update_server_conn_error4(0) #
                ...
                try:
                    ...
                    print(f"⚠️ Failed to send response to 4th : {addr}: {e}")
                    break

            except socket.timeout:
                # Just a timeout, the connection might still be good
                print(f"⏱️ Timeout waiting for data from 4th :{addr}, but connection maintained.") #
                # Send a keepalive message to check if connection is still alive
                try:
                    client_socket.sendall(b"keepalive")
                    print(f"✅ Keepalive sent to 4th : {addr}")
                except socket.error as e:
                    print(f"⚠️ Connection lost with 4th : {addr} during keepalive: {e}") #
                    break
                continue
                
            except socket.error as e:
                print(f"⚠️ Connection lost with 4th : {addr}: {e}") #
                break

        print(f"🔴 Closing connection with 4th : {addr}") #
        try:
            client_socket.close()
        except Exception as e:
            print(f"⚠️ Error closing socket for 4th : {addr}: {e}")
        ...
        self.database_communication.update_server_conn_error4(1) #

    def run(self):
        """Main server loop to accept and handle clients in separate threads."""
        print("🚀 4th Server is running and waiting for connections...") #
